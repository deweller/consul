...
func testRemoteExecGetSpec(t *testing.T, c *Config) {
	...
	event := &remoteExecEvent{
		Prefix:  "_rexec",
		Session: makeRexecSession(t, a),
	}
	defer destroySession(t, a, event.Session)
...
func makeRexecSession(t *testing.T, a *TestAgent) string {
...
func destroySession(t *testing.T, a *TestAgent, session string) {
	args := structs.SessionRequest{
		Datacenter: a.config.Datacenter,
...
func setKV(t *testing.T, a *TestAgent, key string, val []byte) {
...
func getKV(t *testing.T, a *TestAgent, key string) *structs.DirEntry {
