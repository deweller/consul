...
func TestACL_Version8(t *testing.T) {
	c := nextConfig()
	c.ACLEnforceVersion8 = Bool(false)
...
func TestACL_Disabled(t *testing.T) {
	c := nextConfig()
	c.ACLDisabledTTL = 10 * time.Millisecond
...
func TestACL_Special_IDs(t *testing.T) {
	...
	if !acl.AgentRead(c.NodeName) {
		t.Fatalf("should be able to read agent")
	}
	if !acl.AgentWrite(c.NodeName) {
